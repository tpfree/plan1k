...
<form method="post"
	action="<c:url value='declarationInfo.do?delcare&callbackType=forward'/>"
	...
	<div class="pageContent">
		<div class="pageFormContent" layoutH="97">
			...
			<dl class="nowrap">
				...
				<dd>
					<select class="required" name="declareFields" class="required">
						...
						<option value="1" <c:if test="${declare.declareFields==1 }">selected</c:if> >生态环保</option>
						<option value="2" <c:if test="${declare.declareFields==2 }">selected</c:if>>企业技术</option>
						<option value="3" <c:if test="${declare.declareFields==3 }">selected</c:if>>企业管理</option>
						<option value="4" <c:if test="${declare.declareFields==4 }">selected</c:if>>创业</option>
						<option value="5" <c:if test="${declare.declareFields==5 }">selected</c:if>>科技创新</option>
						<option value="6" <c:if test="${declare.declareFields==6 }">selected</c:if>>社会科学</option>
						<option value="7" <c:if test="${declare.declareFields==7 }">selected</c:if>>医疗卫生</option>
						<option value="8" <c:if test="${declare.declareFields==8 }">selected</c:if>>文化艺术</option>
						<option value="9" <c:if test="${declare.declareFields==9 }">selected</c:if>>农林技术推广</option>
						<option value="10" <c:if test="${declare.declareFields==10 }">selected</c:if>>社会工作</option>
						<option value="11" <c:if test="${declare.declareFields==11 }">selected</c:if>>其他</option>
					</select>
				</dd>
			</dl>
			<div class="divider"></div>
			<dl class="nowrap">
				<dt>申报单位：</dt>
				<dd>
					<select name="declareUnit" class="required">
						<option value="">-请选择-</option>
